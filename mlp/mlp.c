...
double Vc1, Vc2, Vc3, Vl1, Vl2, Vl3, Vcf1, Vcf2, Vcf3;  // probability with which participants choose method of changing efforts and tax levels
...
int read_config(char *file_name)
{
  ...
  EXPECT(1, "double   Vcf1      = %lf;", &Vcf1);
  EXPECT(1, "double   Vcf2      = %lf;", &Vcf2);
  EXPECT(1, "double   Vcf3      = %lf;", &Vcf3);
 ...
 */
{
  sprintf(fname, "g%02dn%02db%0.2fB%.2ftu%.2ftd%.2feu%.2fed%.2fcx%.2fcy%.2fcz%.2fv1%.2fv2%.2fv3%.2fY0%.2fX0%.2fe%.2fE%.2fx0%.2fy0%.2fz0%.2f%s", G, n, b, B, Theta_u, Theta_d, Eta_u, Eta_d, cx, cy, cz, Vc1, Vc2, Vc3, Y0, X0, e, E, x_0, y_0, z_0, apndStr);
}
...
{ 
  ...
  sprintf(title, "b:%.1f, B:%.1f t_u:%.1f, t_d:%.1f, e_u:%.1f, e_d%.1f, c:%.1f,%.1f,%.1f, v:%d%d%d, e:%.1f, E:%.1f, x0:%.2f, y0:%.2f, z0:%.2f, Y0:%.2f", b, B, Theta_u, Theta_d, Eta_u, Eta_d, cx, cy, cz, (int)(Vc1*10), (int)(Vc2*10), (int)(Vc3*10), e, E, x_0, y_0, z_0, Y0);
  fprintf(gp, "set key outside vertical  spacing 1 width 1 height -2\n");        
  if(m){ // save graphs as file
    sprintf(xpng, "g%02dn%02db%0.2fB%.2ftu%.2ftd%.2feu%.2fed%.2fcx%.2fcy%.2fcz%.2fY0%.2fe%.2fE%.2fx0%.2fy0%.2fz0%.2fv%d%d%d.png", G, n, b, B, Theta_u, Theta_d, Eta_u, Eta_d, cx, cy, cz, Y0, e, E, x_0, y_0, z_0, (int)(Vc1*10), (int)(Vc2*10), (int)(Vc3*10)); 
 ...
 */
{
  ...
  fprintf(gp, "set key outside vertical height -1\n");        
  if(m){ // save graphs as file
    sprintf(xpng, "g%02dn%02db%0.2fB%.2ftu%.2ftd%.2feu%.2fed%.2fcx%.2fcy%.2fcz%.2fY0%.2fe%.2fE%.2fx0%.2fy0%.2fz0%.2fv%d%d%d_%d.png", G, n, b, B, Theta_u, Theta_d, Eta_u, Eta_d, cx, cy, cz, Y0, e, E, x_0, y_0, z_0, (int)(Vc1*10), (int)(Vc2*10), (int)(Vc3*10), r); 
    fprintf(gp, "set term pngcairo size 1024,768 enhanced color solid font \"Helvetica,8\" \n");
    fprintf(gp, "set output '%s' \n", xpng);
  }
  else{
...
int f(int x, double X, double y, double pn, int n, double t_u, double ppc)
{  
    double p0, p1;                                        // new sum of efforts of commoners for the group j    
...
{  
  ...
  if(v == 1){                                                               // random mutation
    ...
    com->x = f(com->x, g->X, y, pn, g->n, g->t_u, ppc);
  }
}
 ...
 */
{
  ...
  // begin region: v = 3 
    ...
    if(f > ld->pi){                                    // if new payoff is higher than current payoff, then change to new strategies    
      //if(pn > ld->p && n_x0 == 1) printf("pi: %.6lf  f: %.6lf pun_cost: %.6lf n_x0: %d p: %.6lf pn: %.6lf \n", ld->pi, f, (pn*delta*n_x0), n_x0, ld->p, pn);
      ld->y = y;
      ld->p = pn;       
      //printf("p changed\n");
    }    
